...
| C3.2.2 | API actions for creating, rescheduling, completing, and cancelling follow-ups | C3.2.1 | Nested route endpoints protect ownership scoping (routing Lead A with B's follow-up returns 404); validation requires `after_or_equal:now`; rescheduling/completing/cancelling clears snooze; terminal states cannot reopen (422 on second action); nested user details returned; writes wrapped in DB::transaction | Authorization, validation, scoped bindings, state logic, terminal state immutability, and JSON schema tests | CRM, API | High |
| C3.2.3 | Listing API with query filters for due-today/overdue views, assignees, and status | C3.2.2 | Global list endpoint `GET /admin/leads/follow-ups` eager loads relations (prevents N+1); request validates status/filter as mutually exclusive; sorted by `orderBy('due_at')->orderBy('id')` ascending; preserves query string in pagination | Pagination, sorting, empty result, combined filters, mutual exclusivity, status, overdue, and due_today views tests | CRM, API | High |
| C3.2.4 | Scheduled Artisan command `crm:dispatch-follow-up-reminders` and `LeadFollowUpDue` handoff event | C3.2.3 | Command queries due/overdue pending follow-ups with `due_at <= now()`; uses `->lazy()` to stream memory-efficiently; dispatches event per item; registered to run every minute in `console.php` | Event dispatch, boundary logic, completed/cancelled ignore, and deterministic ordering tests | CRM, Scheduler | Medium |
| C3.2.5 | Lead activity timeline integration | C3.2.3 | Create, update, complete, and cancel actions write timeline entries to `lead_activities` inside DB transactions; metadata holds domain-specific keys | Activity creation, reschedule detection with date comparison, and metadata format tests | CRM, Database | High |
| C3.2.6 | Follow-up permissions and retry-safe regression tests | C3.2.5 | Gated authorization matrix is enforced based on permissions (`leads.manage`/`leads.view`); scoped bindings route protection prevents cross-lead follow-up access; terminal states are retry-safe (422 response) and do not emit double activities; terminal states ignore command reminder dispatches | Auth matrix, scoping mismatch, terminal retry, duplicate logs check, command dispatch check | CRM, API | High |
