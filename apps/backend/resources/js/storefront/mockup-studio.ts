  ...
  if (form && canvas && ctx && products.length) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const uploadResponse = await fetch(`${webBase}/products/${encodeURIComponent(productSelect.value)}/design-upload`, { method: 'POST', body: upload, credentials: 'include', headers: { Accept: 'application/json', 'X-CSRF-TOKEN': csrf } });
        if ([401, 403, 419].includes(uploadResponse.status)) throw new Error(`SIGN_IN:${webBase}/login`);
        ...
        const mockupResponse = await fetch(`${webBase}/products/${encodeURIComponent(productSelect.value)}/protected-mockup/${encodeURIComponent(sourceId)}`, {
          method: 'POST',
          credentials: 'include',
    ...
    addCartButton.addEventListener('click', async () => {
      ...
      setError();
      try {
        const response = await fetch(`${webBase}/cart/items`, {
          method: 'POST',
          credentials: 'include',
          headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf },
