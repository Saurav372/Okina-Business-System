<x-layouts.admin title="Stock Balances">
    <div class="space-y-6" x-data="{
        ...
        <div class="bg-white border border-neutral-200 rounded-2xl p-4 sm:p-5 shadow-xs">
            <form method="GET" action="{{ route('admin.inventory.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <!-- Search Input -->
                            ...
                            <tr class="hover:bg-neutral-50/60 transition-colors">
                                ...
                                <td class="py-3.5 px-4 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        @endcan
                                        <a href="{{ route('admin.inventory.movements.index') }}?search={{ $item->sku?->sku_code }}" class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-neutral-700 bg-white hover:bg-neutral-50 border border-neutral-300 rounded-lg shadow-2xs transition-colors">
        ...
        <div x-show="adjustModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div @click="closeAdjustModal()" class="fixed inset-0 bg-neutral-950/40 backdrop-blur-xs transition-opacity" aria-hidden="true"></div>
                ...
                <div class="inline-block align-bottom bg-white border border-neutral-200 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <template x-if="activeItem">
                        <form :action="`/admin/inventory/${activeItem.sku_id}/adjust`" method="POST" class="p-6 space-y-4">
                            @csrf
                            ...
                            <!-- On-Hand Quantity Adjustment -->
                            <div>
                                <label class="block text-xs font-semibold text-neutral-700 mb-1">New Physical On-Hand Quantity</label>
                                <input type="number" name="on_hand_quantity" min="0" x-model.number="newOnHand" required class="w-full px-3.5 py-2 border border-neutral-300 rounded-xl text-xs text-neutral-900 font-mono focus:outline-none focus:ring-2 focus:ring-[color:var(--focus-ring-color)]">
                                <div class="flex justify-between text-[11px] text-neutral-500 mt-1">
                                    <span>Current: <strong x-text="activeItem.on_hand_quantity"></strong></span>
