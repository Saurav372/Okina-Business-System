...
class AdminDesignFileAccessTest extends TestCase
{
    use RefreshDatabase;

    public function test_admin_detail_includes_signed_mockup_preview_url_when_snapshot_has_preview_metadata(): void
    {
        ...
        // Create order and order item with customization snapshot containing a mockup preview
        $customer = Customer::factory()->create();

        $order = Order::query()->create([
            'public_id' => 'OD-DESIGN-001',
        ...
        $itemSnapshot = $summary['items'][0]['customization_snapshot'];

        $this->assertArrayHasKey('mockup_preview_url', $itemSnapshot);
        $this->assertIsString($itemSnapshot['mockup_preview_url']);
        $this->assertStringContainsString('/api/catalog/products/snapshot-product-one/design-preview/SF-TEST-001', $itemSnapshot['mockup_preview_url']);
        $this->assertStringContainsString('signature=', $itemSnapshot['mockup_preview_url']);
    }
}
