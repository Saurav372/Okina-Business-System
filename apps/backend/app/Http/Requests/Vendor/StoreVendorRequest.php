...
class StoreVendorRequest extends FormRequest
{
    ...
    {
        $this->merge([
            'vendor_code' => filled($this->vendor_code) ? strtoupper(trim($this->vendor_code)) : null,
    ...
    public function rules(): array
    {
        return [
            ...
            'vendor_code' => ['nullable', 'string', 'max:60', 'regex:/^[A-Z0-9]+(?:-[A-Z0-9]+)*$/', Rule::unique('vendors', 'vendor_code')],
            'status' => ['nullable', 'string', Rule::enum(VendorStatus::class)],
            'contact_name' => ['nullable', 'string', 'max:120'],
            'email' => ['nullable', 'email', 'max:180'],
            'phone' => ['nullable', 'string', 'max:40'],
            'gstin' => ['nullable', 'string', 'max:30', 'regex:/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/', Rule::unique('vendors', 'gstin')],
